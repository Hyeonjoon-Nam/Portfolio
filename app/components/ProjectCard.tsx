      ...
      <div className="p-4">
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          {p.roles.slice(0, 3).map((r) => (
            <span key={r} className="text-xs px-2 py-0.5 rounded-full bg-white/10 border border-white/10">
              {r}
            </span>
          ))}
        </div>

        <div className="mt-3 text-sm underline underline-offset-4 text-white/80">
          View details →
        </div>
      </div>
    </Link>
